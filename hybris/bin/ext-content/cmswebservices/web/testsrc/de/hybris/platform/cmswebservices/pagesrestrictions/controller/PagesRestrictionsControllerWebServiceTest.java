...
public class PagesRestrictionsControllerWebServiceTest extends ApiBaseIntegrationTest
{
	...
	public void setup()
	{
		...
		final AbstractPageModel emailPage = contentPageModelMother.EmailPageFromEmailPageTemplate(catalogVersion);
		contentPageModelMother.somePage(catalogVersion, OTHER_PAGE, "other-page-title");

		timeRestrictionModelMother.today(catalogVersion, homePage, emailPage);
	...
	{
		final Response responseForEmail = getCmsManagerWsSecuredRequestBuilder() //
				.path(replaceUriVariablesWithDefaults(PAGE_RESTRICTION_ENDPOINT, new HashMap<>()))
				.queryParam(CmswebservicesConstants.URI_PAGE_ID, ContentPageModelMother.UID_EMAILPAGE).build() //
				.accept(MediaType.APPLICATION_JSON) //
				.get();

		assertResponse(Status.OK, responseForEmail);

		final PageRestrictionListData emailPageRestrictionListEntity = responseForEmail.readEntity(PageRestrictionListData.class);
		assertThat(emailPageRestrictionListEntity.getPageRestrictionList().size(), is(1));
		assertThat(emailPageRestrictionListEntity.getPageRestrictionList().get(0).getPageId(),
				is(ContentPageModelMother.UID_EMAILPAGE));
		assertThat(emailPageRestrictionListEntity.getPageRestrictionList().get(0).getRestrictionId(),
	...
	public void shouldFailPutPageRestrictions_AmbiguousPageId() throws Exception
	{
		...
		final PageRestrictionListData entity = createAnEntityForRestrictionOnPage(
				ContentPageModelMother.UID_EMAILPAGE, CMSTimeRestrictionModelMother.UID_TOMORROW);
		final Response responseForPutOnHomepage = getCmsManagerWsSecuredRequestBuilder() //
				.path(replaceUriVariablesWithDefaults(PAGE_RESTRICTION_UPDATE_ENDPOINT, collectionInstanceMap))
				.build() //
				.accept(MediaType.APPLICATION_JSON) //
	...
	public void shouldGetAllPagesRestrictions()
	{
		...
		final List<String> emailpageRestrictionIds = dataMap.get(ContentPageModelMother.UID_EMAILPAGE).stream()
				.map(page -> page.getRestrictionId()).collect(Collectors.toList());
		...
		assertThat(emailpageRestrictionIds, containsInAnyOrder(CMSTimeRestrictionModelMother.UID_TODAY));
	}
	...
	public void shouldGetPagesRestrictionsForPageIds()
	{
		final Response response = getCmsManagerWsSecuredRequestBuilder() //
				...
				.queryParam(CmswebservicesConstants.URI_PAGE_IDS,
						ContentPageModelMother.UID_HOMEPAGE + "," + ContentPageModelMother.UID_EMAILPAGE)
				.build() //
				.accept(MediaType.APPLICATION_JSON) //
				.get();
	...
	public void shouldGetEmptyPagesRestrictionsForPageIds_OneInvalidPageId()
	{
		final Response response = getCmsManagerWsSecuredRequestBuilder() //
				.path(replaceUriVariablesWithDefaults(PAGE_RESTRICTION_ENDPOINT, new HashMap<>()))
				.queryParam(CmswebservicesConstants.URI_PAGE_IDS, INVALID_ID + "," + ContentPageModelMother.UID_EMAILPAGE).build() //
				.accept(MediaType.APPLICATION_JSON) //
				.get();

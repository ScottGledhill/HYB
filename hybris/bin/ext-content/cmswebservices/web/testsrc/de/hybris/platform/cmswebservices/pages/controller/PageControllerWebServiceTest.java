...
public class PageControllerWebServiceTest extends ApiBaseIntegrationTest
{
	...
	@Test
	public void shouldCreateEmailPage() throws JAXBException
	{
		final EmailPageData page = populatePage(EmailPageModel._TYPECODE, new EmailPageData());
		page.setFromName(getLocalizedContent(FROM_NAME));
		page.setFromEmail(getLocalizedContent(FROM_EMAIL));

		final Response response = getCmsManagerWsSecuredRequestBuilder() //
				.path(replaceUriVariablesWithDefaults(BASE_PAGE_ENDPOINT, new HashMap<>())).build() //
				.accept(MediaType.APPLICATION_JSON) //
				.post(Entity.entity(marshallDto(page, EmailPageData.class), MediaType.APPLICATION_JSON));

		assertResponse(Status.CREATED, response);

		final EmailPageData entity = response.readEntity(EmailPageData.class);
		assertThat(entity,
				allOf(hasProperty(PageFields.UID.property, is(PAGE_UID)), //
						hasProperty(PageFields.NAME.property, is(PAGE_NAME)), //
	...
	public void shouldFailUpdatePage_DuplicateLabelForPrimaryPage() throws JAXBException
	{
		...
		page.setLabel(ContentPageModelMother.LABEL_EMAILPAGE);

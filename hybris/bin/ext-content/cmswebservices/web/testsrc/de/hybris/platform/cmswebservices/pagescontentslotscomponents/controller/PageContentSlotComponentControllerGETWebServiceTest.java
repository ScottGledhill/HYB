...
import static de.hybris.platform.cmsfacades.util.models.ContentSlotModelMother.UID_LOGO;
import static de.hybris.platform.cmsfacades.util.models.SimpleBannerComponentModelMother.UID_HEADER_LOGO;
...
public class PageContentSlotComponentControllerGETWebServiceTest extends ApiBaseIntegrationTest
{
	...
	public void shouldGetComponentsByPage() throws Exception
	{
		...
		final PageContentSlotComponentData bannerHeaderData = entity.getPageContentSlotComponentList().stream() //
				.filter(item -> item.getSlotId().equals(UID_HEADER)) //
				.filter(item -> item.getComponentId().equals(UID_HEADER_LOGO)) //
				.findFirst().get();
		assertThat(bannerHeaderData.getPageId(), Matchers.is(UID_HOMEPAGE));
		assertThat(bannerHeaderData.getSlotId(), Matchers.is(UID_HEADER));
		assertThat(bannerHeaderData.getComponentId(), Matchers.is(UID_HEADER_LOGO));
		assertThat(bannerHeaderData.getPosition(), Matchers.is(1));

		final PageContentSlotComponentData bannerLogoData = entity.getPageContentSlotComponentList().stream() //
				.filter(item -> item.getSlotId().equals(UID_LOGO)) //
				.findFirst().get();
		assertThat(bannerLogoData.getPageId(), Matchers.is(UID_HOMEPAGE));
		assertThat(bannerLogoData.getSlotId(), Matchers.is(UID_LOGO));
		assertThat(bannerLogoData.getComponentId(), Matchers.is(UID_HEADER_LOGO));
		assertThat(bannerLogoData.getPosition(), Matchers.is(0));
	}

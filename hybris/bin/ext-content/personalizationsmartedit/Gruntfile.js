module.exports = function(grunt) {
    ...
    grunt.initConfig({
        ...
        watch: {
            uitests: {
                files: ['Gruntfile.js', 'web/features/**/*', '*jsTests/ui/**/*'],
                tasks: ['protractorRun'],
                options: {
                    atBegin: true
                }
        ...
        protractor: {
            options: {
                keepAlive: false,
                configFile: "jsTests/ui/common/protractor-conf.js"
            },
            run: {},
            runparallel: {
                options: {
    ...
    grunt.registerTask("protractorRun", 'Executes e2e tests for personalizationsmartedit', function() {
        ...
        if (grunt.file.expand({
            ...
            grunt.log.warn('protractor:run grunt phase was not run since no phantomjs driver found under ' + phantomJSPattern);
        }
    });

    grunt.registerTask("protractorRunParallel", 'Executes ui tests for personalization with multiple browser instances', function() {
        //if npmtestancillary is not present, phantomjs drivers won't be present
        if (grunt.file.expand({
                filter: 'isFile'
            }, phantomJSPattern).length > 0) {
            grunt.task.run('protractor:runparallel');
        } else {
            grunt.log.warn('protractor:runparallel grunt phase was not run since no phantomjs driver found under ' + phantomJSPattern);
        }
    });
    ...
    grunt.registerTask('verify_only', ['uitests']);
    grunt.registerTask('verify', ['package', 'verify_only']);

    grunt.registerTask('full_dev', ['compile', 'package', 'copyDev']);

    grunt.registerTask('uitests_continuous', ['connect:server', 'watch:uitests']);
    grunt.registerTask('uitests', ['connect:server', 'protractorRun']);
    grunt.registerTask('uitests_parallel', ['connect:server', 'protractorRunParallel']);
};

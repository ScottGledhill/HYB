angular.module('personalizationsmarteditManagerModule', [
    ...
    .controller('personalizationsmarteditManagerController', function($scope, hitch, $q, $log, $timeout, personalizationsmarteditManagementService, personalizationsmarteditMessageHandler, personalizationsmarteditUtils, confirmationModalService, $filter, MODAL_BUTTON_ACTIONS, CUSTOMIZATION_VARIATION_MANAGEMENT_TABS_CONSTANTS, CUSTOMIZATION_VARIATION_MANAGEMENT_BUTTONS, systemEventService, CUSTOMIZATION_VARIATION_MANAGEMENT_SEGMENTTRIGGER_GROUPBY, PERSONALIZATION_DATE_FORMATS, PERSONALIZATION_MODEL_STATUS_CODES, personalizationsmarteditCommerceCustomizationService, customizationDataFactory) {
        var self = this;
        ...
        $scope.edit = {
            ...
            viewDateFormat: PERSONALIZATION_DATE_FORMATS.VIEW_DATE_FORMAT,
        ...
        if ($scope.editMode) {
            personalizationsmarteditManagementService.getCustomization($scope.customizationCode).then(function successCallback(response) {
                ...
                $scope.customization.enabledStartDate = personalizationsmarteditUtils.formatDate($scope.customization.enabledStartDate, PERSONALIZATION_DATE_FORMATS.VIEW_DATE_FORMAT);
                $scope.customization.enabledEndDate = personalizationsmarteditUtils.formatDate($scope.customization.enabledEndDate, PERSONALIZATION_DATE_FORMATS.VIEW_DATE_FORMAT);
        ...
        self.onSave = function() {
            if ($scope.customization.enabledStartDate) {
                $scope.customization.enabledStartDate = personalizationsmarteditUtils.formatDate($scope.customization.enabledStartDate, PERSONALIZATION_DATE_FORMATS.MODEL_DATE_FORMAT);
            } else {
                $scope.customization.enabledStartDate = undefined;
            }

            if ($scope.customization.enabledEndDate) {
                $scope.customization.enabledEndDate = personalizationsmarteditUtils.formatDate($scope.customization.enabledEndDate, PERSONALIZATION_DATE_FORMATS.MODEL_DATE_FORMAT);
            } else {
        ...
        self.init = function() {
            ...
            var resetDateTimeConfiguration = function() {
                $scope.customization.enabledStartDate = undefined;
                $scope.customization.enabledEndDate = undefined;
            };
            ...
            $scope.$watch('edit.datetimeConfigurationEnabled', function(datetimeConfigurationEnabled) {
                if (!datetimeConfigurationEnabled) {

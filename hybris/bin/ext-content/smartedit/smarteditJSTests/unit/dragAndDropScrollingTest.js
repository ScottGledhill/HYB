...
describe('dragAndDropScrollingService', function() {
    ...
    // Variables
    var dragAndDropScrollingService, mocks, currentBrowser, scrollAreas, rootScope, windowHeight;
    ...
    beforeEach(function() {
        ...
        var harness = AngularUnitTestHelper.prepareModule('_dragAndDropScrollingModule')
            .mock('browserService', 'getCurrentBrowser').andCallFake(function() {
            ...
            .mock('$window', 'requestAnimationFrame').andReturn(123)
            .mock('$window', 'height').andReturn(windowHeight)
            .mock('$window', 'cancelAnimationFrame').andDoNothing()
            .withTranslations({
                'draganddrop.uihint.top': 'some top message',
                'draganddrop.uihint.bottom': 'some bottom message'
            })
        ...
        scrollAreas = jasmine.createSpyObj('scrollAreas', ['on', 'off', 'appendTo', 'outerHeight', 'height', 'text', 'hide', 'show', 'remove']);

        spyOn(dragAndDropScrollingService, '_getScrollAreas').andReturn(scrollAreas);
        scrollAreas.appendTo.andReturn(scrollAreas);
    });
    ...
    describe('initialize', function() {
        ...
        beforeEach(function() {
            ...
            spyOn(dragAndDropScrollingService, '_addEventListeners').andCallThrough();
            spyOn(dragAndDropScrollingService, '_getSelector').andReturn(selector);
        });
        ...
        it('GIVEN the current browser is Chrome THEN the scrollable area is the body', function() {
            // Arrange
            expect(dragAndDropScrollingService._getSelector).not.toHaveBeenCalled();
            currentBrowser = SUPPORTED_BROWSERS.CHROME;

            // Act
            dragAndDropScrollingService._initialize();

            // Assert
            expect(dragAndDropScrollingService._getSelector).toHaveBeenCalledWith('body');
        });

        it('GIVEN the current browser is FIREFOX THEN the scrollable area is the html', function() {
            // Arrange
            expect(dragAndDropScrollingService._getSelector).not.toHaveBeenCalled();
            currentBrowser = SUPPORTED_BROWSERS.FIREFOX;

            // Act
            dragAndDropScrollingService._initialize();

            // Assert
            expect(dragAndDropScrollingService._getSelector).toHaveBeenCalledWith('html');
        });

        it('GIVEN the current browser is IE THEN the scrollable area is the html', function() {
            // Arrange
            expect(dragAndDropScrollingService._getSelector).not.toHaveBeenCalled();
            currentBrowser = SUPPORTED_BROWSERS.IE;
    ...
    it('WHEN _addScrollAreas is called THEN scroll areas are added', function() {
        ...
        spyOn(dragAndDropScrollingService, '_getSelector').andCallFake(function(arg) {
        ...
        topArea.appendTo.andReturn(topArea);
        bottomArea.appendTo.andReturn(bottomArea);
    ...
    it('WHEN mouse enters the UI scrolling hints THEN animation starts', function() {
        ...
        spyOn(dragAndDropScrollingService, '_getSelector').andReturn(scrollArea);
        spyOn(dragAndDropScrollingService, '_scrollPage').andCallFake(function() {
            isFnCalled = true;
        });
        ...
        // Assert
        var callback = mocks.$window.requestAnimationFrame.calls[0].args[0];
        callback();
    ...
    describe('on mouse over', function() {
        ...
        it('WHEN mouse is over top slow area THEN the page scrolls up slowly', function() {
            ...
            scrollArea.attr.andReturn('top_scroll_page');
        ...
        it('WHEN mouse is over bottom slow area THEN the page scrolls down slowly', function() {
            ...
            scrollArea.attr.andReturn('bottom_scroll_page');
    ...
    describe('_scrollPage', function() {
        ...
        it('GIVEN the page is at the top WHEN _scrollPage is called to scroll up THEN scrolling is not executed', function() {
            ...
            dragAndDropScrollingService._scrollable.scrollTop.andReturn(0);
            spyOn(dragAndDropScrollingService, '_showScrollAreas');
        ...
        it('GIVEN the page is at the bottom WHEN _scrollPage is called to scroll down THEN scrolling is not executed', function() {
            // Arrange
            dragAndDropScrollingService._scrollDelta = 10;
            dragAndDropScrollingService._scrollable.scrollTop.andReturn(scrollLimit);
            spyOn(dragAndDropScrollingService, '_showScrollAreas');
        ...
        it('GIVEN the page is not at the top WHEN _scrollPage is called to scroll up THEN scrolling is executed', function() {
            // Arrange
            dragAndDropScrollingService._scrollDelta = -10;
            dragAndDropScrollingService._scrollable.scrollTop.andReturn(10);
            spyOn(dragAndDropScrollingService, '_showScrollAreas');
        ...
        it('GIVEN the page is not at the bottom WHEN _scrollPage is called to scroll down THEN scrolling is executed', function() {
            // Arrange
            dragAndDropScrollingService._scrollDelta = 10;
            dragAndDropScrollingService._scrollable.scrollTop.andReturn(scrollLimit - 10);
            spyOn(dragAndDropScrollingService, '_showScrollAreas');
    ...
    it('WHEN enable is called THEN the scroll areas are shown', function() {
        ...
        dragAndDropScrollingService._scrollable = jasmine.createSpyObj('scrollable', ['height', 'outerHeight', 'scrollTop']);

        spyOn(dragAndDropScrollingService, '_getSelector').andReturn(mocks.$window);
        spyOn(dragAndDropScrollingService, '_showScrollAreas');

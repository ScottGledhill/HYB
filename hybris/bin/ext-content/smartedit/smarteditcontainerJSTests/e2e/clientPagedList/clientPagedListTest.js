...
describe('clientPagedList - ', function() {
    ...
    it('GIVEN I am on the page list WHEN I click on more menu button THEN I expect to open a drop down with the list of actions', function() {
    ...
    it('GIVEN I am on the page list WHEN I click on more menu button AND click on the first one THEN I expect to execute a callback function to remove the items', function() {
        var el = list.openMoreMenuFirstElement();
        var dropDown = el.all(by.css('.dropdown-menu > li'));
        dropDown.get(0).getText().click();
        expect(list.totalPageCount().getText()).toBe("(0 pagelist.countsearchresult)");
    });


    // Helper functions
    function navigateToFirstCatalogPageList() {
        return browser.click(element(by.css('.catalog-container .page-list-link-item a')));
    }

 ...
 */
angular.module('renderServiceModule', ['functionsModule', 'gatewayProxyModule', 'renderServiceInterfaceModule', 'alertServiceModule', 'decoratorServiceModule', 'sakExecutorDecorator', 'perspectiveServiceModule', 'gatewayFactoryModule', 'componentHandlerServiceModule', 'crossFrameEventServiceModule'])
...
.service('renderService', function($q, $compile, $rootScope, $http, $location, $window, alertService, componentHandlerService, decoratorService, extend, extractFromElement, gatewayFactory, gatewayProxy, hitch, isBlank, sakExecutor, perspectiveService, renderGateway, RenderServiceInterface, unsafeParseHTML, crossFrameEventService, COMPONENT_CLASS, ID_ATTRIBUTE, OVERLAY_COMPONENT_CLASS, OVERLAY_ID, SMARTEDIT_ATTRIBUTE_PREFIX, TYPE_ATTRIBUTE, OVERLAY_RERENDERED_EVENT) {
    ...
    this._resizeSlots = function() {
        this._getFirstChildren('body').each(function(index, slotComponent) {
            ...
            var currentSlotHeight = parseFloat(window.getComputedStyle(slotComponent).height);
    ...
    this._getFirstChildren = function(seComponent) {
        ...
        return stem.find("." + COMPONENT_CLASS).filter(function(index) {
            var match = componentHandlerService.getFromSelector(this).getCssPath().replace(parentCssPath, "").match(firstChildrenRegex);

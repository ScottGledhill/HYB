...
public class DefaultPageContentSlotComponentFacade implements PageContentSlotComponentFacade
{
	...
	{
		final List<AbstractCMSComponentModel> components = Lists
			...
			final Integer position, final String componentId)
	{
		...
		pageContentSlotComponent.setComponentId(componentId);
			...
			throws CMSItemNotFoundException, ValidationException
	{
		...
		final AbstractCMSComponentModel component = getAdminComponentService()
				.getCMSComponentForId(pageContentSlotComponent.getComponentId());

		try
		{
			final ContentSlotModel contentSlot = getAdminContentSlotService()
					.getContentSlotForId(pageContentSlotComponent.getSlotId());
			...
			throws CMSItemNotFoundException, ComponentNotFoundInSlotException
	{
		...
		component = fetchComponent(componentId);

		contentSlot = fetchContentSlot(slotId);
			...
			throws CMSItemNotFoundException, ComponentNotAllowedInSlotException, ValidationException
	{
		...
		final ContentSlotModel originSlot = fetchSlot(originSlotUid);
		...
		{
			final ContentSlotModel destSlot = fetchSlot(destSlotUid);
	...
	protected ContentSlotModel fetchSlot(final String originSlotUid) throws CMSItemNotFoundException
	{
		return fetchContentSlot(originSlotUid);
	}
	...
	protected AbstractCMSComponentModel fetchComponent(final String componentId) throws CMSItemNotFoundException
	{
		try
		{
			return getAdminComponentService().getCMSComponentForId(componentId);
		}
		catch (AmbiguousIdentifierException | UnknownIdentifierException e)
		{
			throw new CMSItemNotFoundException("Component Item with Uid '" + componentId + "' not found.");
		}
	}
	...
	protected ContentSlotModel fetchContentSlot(final String slotId) throws CMSItemNotFoundException
	{
		try
		{
			return getAdminContentSlotService().getContentSlotForId(slotId);
		}
		catch (AmbiguousIdentifierException | UnknownIdentifierException e)
		{
			throw new CMSItemNotFoundException("Content slot with Uid '" + slotId + "' not found.");
		}
	}

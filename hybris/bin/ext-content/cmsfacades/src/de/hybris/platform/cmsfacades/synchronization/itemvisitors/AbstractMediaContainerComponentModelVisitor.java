...
/**
 * Abstract class for visiting {@link AbstractMediaContainerComponentModel} models for the cms synchronization service to work properly. 
 ...
 *
 * @param <MEDIACONTAINERTYPE> the media container type that extends {@link AbstractMediaContainerComponentModel}
 */
public abstract class AbstractMediaContainerComponentModelVisitor<MEDIACONTAINERTYPE extends AbstractMediaContainerComponentModel> extends AbstractCMSComponentModelVisitor<MEDIACONTAINERTYPE>
{

	@Override
	public List<ItemModel> visit(MEDIACONTAINERTYPE source, List<ItemModel> path, Map<String, Object> ctx)
	{
		...
		Optional.ofNullable(ctx.get(VISITORS_CTX_LOCALES)) //
				...
				.ifPresent(locales -> locales //
						.stream() //
						.map(locale -> source.getMedia(locale))
						.filter(mediaContainer ->  mediaContainer != null)

...
public class DefaultItemSynchronizationFacade implements ItemSynchronizationFacade
{
			...
			final SyncItemStatusConfig config)
	{
		...
		ItemModel item = getItem(itemSynchronizationData);
	...
	{
		List<ItemModel> items = getListOfItems(synchronizationData);
	 ...
	 */
	protected List<ItemModel> getListOfItems(SynchronizationData synchronizationData)
	{
			...
			final SyncItemStatusConfig config, final Integer level) 
	{
		...
		syncItemStatusData
				.setSelectedDependencies(collectItemsAndGetSynchronizationItemStatus(syncRequestData, item, config, level + 1, getBasicItemCollectorRegistry()));

		syncItemStatusData
				.setSharedDependencies(collectItemsAndGetSynchronizationItemStatus(syncRequestData, item, config, level + 1, getSharedItemCollectorRegistry()));
	 ...
	 */
	protected List<SyncItemStatusData> collectItemsAndGetSynchronizationItemStatus(final SyncRequestData syncRequestData, final ItemModel item,
			final SyncItemStatusConfig config, final Integer level, final ItemCollectorRegistry collectorRegistry)
	{
		...
		final Function<ItemModel, SyncItemStatusData> performGetSynchronizationStatus = 
				itemModel -> getSynchronizationItemStatus(syncRequestData, itemModel, config, level);
		
		return itemsCollected
				.stream() //
				.map(performGetSynchronizationStatus).collect(toList());
	}

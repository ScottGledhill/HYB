...
public class CreateCatalogSynchronizationValidator implements Validator
{
	...
	private static final String TARGET_NAME_ATTR = "targetVersionId";
	...
	public void validate(final Object target, final Errors errors)
	{
		...
		try
		{
			...
			final CatalogVersionModel targetCatalog = getCatalogVersionModel(syncJobRequestData.getCatalogId(),
					syncJobRequestData.getTargetVersionId());

			final UserModel userModel = getUserService().getCurrentUser();
			final boolean canRead = getCatalogVersionService().canRead(sourceCatalog, userModel);
			final boolean canWrite = getCatalogVersionService().canWrite(targetCatalog, userModel);

			if (!canRead)
			{
				errors.rejectValue(SOURCE_NAME_ATTR, UNAUTHORIZED_SYNCHRONIZATION_READ);
			}
			if (!canWrite)
			{
				errors.rejectValue(TARGET_NAME_ATTR, UNAUTHORIZED_SYNCHRONIZATION_WRITE);
			}
		}

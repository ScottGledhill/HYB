 ...
 */
public class MediaContainerDataPopulator implements Populator<MediaContainerData, MediaContainerModel>
{
	...
	@Override
	public void populate(final MediaContainerData source, final MediaContainerModel target) throws ConversionException
	{
		...
		return (formatMediaCodeMap) -> {
			...
			Optional.ofNullable(formatMediaCodeMap).ifPresent(map -> {
				...
				map.entrySet().stream().forEach(entry -> {
					...
					final MediaFormatModel mediaFormat = mediaFormats.get(format);

					getMediaModel(mediaCode).ifPresent(media -> {
	 ...
	 * @param medias the original media list on the target media container
	 * @param formatMediaCodeMap the new format/media map being requested for change
	 * @return {@code true} if the media list has been modified; {@code false} otherwise. 
	 */
	protected boolean isFormatMediaMapTheSameAsBefore(final Collection<MediaModel> medias, final Map<String, String> formatMediaCodeMap)
	{
		if (medias == null) 
		{
			return false;
		}
		final Map<String, String> originalMap = medias.stream().collect(Collectors.toMap(media -> media.getMediaFormat().getQualifier(), media -> media.getCode(), (entry1, entry2) -> entry1));

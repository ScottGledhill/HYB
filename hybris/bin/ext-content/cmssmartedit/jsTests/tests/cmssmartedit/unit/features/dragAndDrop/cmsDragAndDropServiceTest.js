...
describe('cmsDragAndDropService', function() {
    ...
    var gateway, gatewayFactory, $rootScope, $q;
    ...
    describe('register', function() {
        ...
        it('WHEN register is called THEN the right configuration is stored in the base drag and drop service.', function() {
            ...
            var arg = mocks.dragAndDropService.register.calls[0].args[0];
            ...
            expect(arg.sourceSelector).toEqual(["#smarteditoverlay .smartEditComponentX[data-smartedit-component-type!='ContentSlot'] .movebutton",
                ".smartedit-html-container ul[id*='more-menu'] .movebutton"
            ]);
        ...
        it('WHEN register is called THEN the right onStart callback is registered', function() {
            ...
            spyOn(cmsDragAndDropService, 'onStart').andReturn(expectedResult);
        ...
        it('WHEN register is called THEN the right onStop callback is registered', function() {
            // Arrange
            var expectedResult = 'someResult';
            spyOn(cmsDragAndDropService, 'onStop').andReturn(expectedResult);
        ...
        it('WHEN register is called THEN the right onDragEnter callback is registered', function() {
            // Arrange
            var expectedResult = 'someResult';
            spyOn(cmsDragAndDropService, 'onDragEnter').andReturn(expectedResult);
        ...
        it('WHEN register is called THEN the right onDragOver callback is registered', function() {
            // Arrange
            var expectedResult = 'someResult';
            spyOn(cmsDragAndDropService, 'onDragOver').andReturn(expectedResult);
        ...
        it('WHEN register is called THEN the right onDrop callback is registered', function() {
            // Arrange
            var expectedResult = 'someResult';
            spyOn(cmsDragAndDropService, 'onDrop').andReturn(expectedResult);
    ...
    it('WHEN unregister is called THEN the service is cleaned.', function() {
        ...
        spyOn(cmsDragAndDropService, '_onOverlayUpdate').andReturn(expectedResult);
        ...
        var overlayRenderedCallbackResult = mocks.systemEventService.unRegisterEventHandler.calls[0].args[1]();
        var componentRemovedCallbackResult = mocks.systemEventService.unRegisterEventHandler.calls[1].args[1]();
    ...
    describe('apply', function() {
        ...
        it('WHEN drop is stopped from the outer frame THEN the right callback is called.', function() {
            ...
            var data = 'some data';
            cmsDragAndDropService.apply();
            var callback = gateway.subscribe.calls[1].args[1];
            callback(eventId, data);
    ...
    describe('event handlers', function() {
        ...
        beforeEach(function() {
            ...
            component.closest.andCallFake(function(arg) {
            ...
            mocks.componentHandlerService.getSlotOperationRelatedId.andReturn(componentId);
            mocks.componentHandlerService.getSlotOperationRelatedType.andReturn(componentType);
            mocks.componentHandlerService.getId.andReturn(slotId);
        ...
        it('WHEN onStart is called THEN it prepares the page for the drag operation', function() {
            ...
            spyOn(cmsDragAndDropService, '_initializeDragOperation');
            spyOn(cmsDragAndDropService, '_getSelector').andReturn(component);
        ...
        it('GIVEN the cursor is over a slot and the hints are already highlighted WHEN onDragOver is called THEN nothing is done', function() {
            // Arrange
            spyOn(cmsDragAndDropService, '_isMouseInRegion').andReturn(true);
        ...
        it('GIVEN the cursor is over a slot and the hint changes WHEN onDragOver is called THEN the hints are updated', function() {
            // Arrange
            spyOn(cmsDragAndDropService, '_isMouseInRegion').andCallFake(function(evt, item) {
        ...
        it('GIVEN the cursor is over a slot and the component changes WHEN onDragOver is called THEN the component hints are updated', function() {
            ...
            spyOn(cmsDragAndDropService, '_clearHighlightedHint').andCallThrough();
            spyOn(cmsDragAndDropService, '_clearHighlightedComponent').andCallThrough();
        ...
        it('GIVEN the cursor is over the dragged element WHEN onDragOver is called THEN the component is highlighted', function() {
            ...
            mocks.componentHandlerService.getSlotOperationRelatedId.andReturn(component.id);
            spyOn(cmsDragAndDropService, '_isMouseInRegion').andCallFake(function(evt, item) {
                if (item === initialValues.hint) {
                    return false;
                } else if (item === initialValues.component) {
        ...
        it('WHEN mouse leaves drag area THEN the slot is cleared', function() {
            // Arrange
            spyOn(cmsDragAndDropService, '_isMouseInRegion').andReturn(false);
    ...
    describe('onDrop', function() {
        var event, slot, dragInfo, targetSlotId;

        beforeEach(function() {
            spyOn(cmsDragAndDropService, '_scrollToModifiedSlot');
            mocks.componentEditingFacade.addNewComponentToSlot.andReturn($q.when());
            mocks.componentEditingFacade.addExistingComponentToSlot.andReturn($q.when());
            mocks.componentEditingFacade.moveComponent.andReturn($q.when());
            ...
            mocks.componentHandlerService.getId.andReturn(targetSlotId);
        ...
        it('WHEN a component is dropped  outside a drop area THEN nothing happens', function() {
            ...
            // Assert
            expect(mocks.alertService.pushAlerts).not.toHaveBeenCalled();
        });
        ...
        it('WHEN a component is dropped in an invalid slot THEN an alert is displayed', function() {
            ...
            var expectedTranslation = 'drag.and.drop.not.valid.component.type';
            var expectedResult = {
                successful: false,
            ...
            // Assert
            expect(mocks.alertService.pushAlerts).toHaveBeenCalledWith([expectedResult]);
        ...
        it('WHEN a new component is dropped THEN the generic editor modal is displayed for it.', function() {
            ...
            // Assert
            expect(mocks.componentEditingFacade.addNewComponentToSlot).toHaveBeenCalledWith(targetSlotId, dragInfo.componentType, expectedPosition);
        ...
        it('WHEN an existing component is dropped into a slot that already has an instance THEN an error message is displayed.', function() {
            // Arrange
            mocks.componentEditingFacade.addExistingComponentToSlot.andReturn($q.reject());
            cmsDragAndDropService._dragInfo.slotId = null;
            cmsDragAndDropService._highlightedHint = highlightedHint;
            var expectedPosition = highlightedHint.position;
            ...
            // Assert
            expect(mocks.componentEditingFacade.addExistingComponentToSlot).toHaveBeenCalledWith(targetSlotId, dragInfo.componentId, expectedPosition);
            expect(cmsDragAndDropService.onStop).toHaveBeenCalled();
        });
        ...
        it('WHEN a new component is moved within the same slot before THEN page is updated accordingly.', function() {
            ...
            spyOn(cmsDragAndDropService, '_getComponentPositionInSlot').andReturn(currentPosition);
            cmsDragAndDropService._highlightedHint = highlightedHint;
            var expectedPosition = highlightedHint.position;
    ...
    it('WHEN _initializeDragOperation is called THEN the page is prepared for dragging components', function() {
        ...
        mocks.componentHandlerService.getOverlay.andReturn(overlay);

...
describe('displayConditionsFacade', function() {
    ...
    beforeEach(function() {
        ...
        var harness = AngularUnitTestHelper.prepareModule('displayConditionsFacadeModule')
            .mock('pageDisplayConditionsService', 'getPageDisplayConditionsDescriptionI18nKeyForPageId').andReturn(resource)
            .mock('pageDisplayConditionsService', 'getPageDisplayConditionsValueForPageId').andReturn(resource)
            .mock('pageRestrictionsService', 'getPageRestrictionsCountForPageUID')
            .mock('pageService', 'getPageById').andReturn(resource)
            .mock('pageService', 'getPrimaryPagesForPageType')
            .mock('pageService', 'getVariationPages')
            .mock('pageService', 'getPrimaryPage').andReturn(resource)
            .mock('pageService', 'isPagePrimary').andReturn(resource)
    ...
    describe('getPageInfoForPageUid', function() {
        beforeEach(function() {
            mocks.pageService.getPageById.andReturn($q.when(MOCK_PAGE));
            mocks.pageService.isPagePrimary.andReturn($q.when(true));
        });
    ...
    describe('getVariationsForPageUid', function() {
        it('will return a promise resolving to an empty array if no variations are found', function() {
            mocks.pageService.getVariationPages.andReturn($q.when([]));
        ...
        it('will return a list of variation pages, each of which having a page name, creation date, and number of restrictions', function() {
            mocks.pageService.getVariationPages.andReturn($q.when(MOCK_VARIATION_PAGES));
            mocks.pageRestrictionsService.getPageRestrictionsCountForPageUID.andCallFake(function(pageUid) {
    ...
    describe('getPrimaryPageForVariationPage', function() {
        beforeEach(function() {
            mocks.pageService.getPrimaryPage.andReturn($q.when(MOCK_PAGE));
        });
    ...
    describe('getPrimaryPagesForVariationPageType', function() {
        beforeEach(function() {
            mocks.pageService.getPrimaryPagesForPageType.andReturn($q.when(MOCK_PRIMARY_PAGES));
        });

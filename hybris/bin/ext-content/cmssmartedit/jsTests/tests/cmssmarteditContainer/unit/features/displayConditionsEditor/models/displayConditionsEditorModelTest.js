...
describe('displayConditionsEditorModel', function() {
    ...
    describe('initModel', function() {
        ...
        it('should put the page name, page, type, and is primary values on the model scope', function() {
            mocks.displayConditionsFacade.getPageInfoForPageUid.andReturn($q.when(MOCK_PRIMARY_PAGE_INFO));
        ...
        it('should delegate to _initModelForVariation if the page is variation', function() {
            mocks.displayConditionsFacade.getPageInfoForPageUid.andReturn($q.when(MOCK_VARIATION_PAGE_INFO));
            service.initModel('somePageUid');
            $rootScope.$digest();
    ...
    describe('_initModelForPrimary', function() {
        beforeEach(function() {
            mocks.displayConditionsFacade.getVariationsForPageUid.andReturn($q.when(MOCK_VARIATIONS));
        });
    ...
    describe('_initModelForVariation', function() {
        beforeEach(function() {
            mocks.displayConditionsFacade.getPrimaryPageForVariationPage.andReturn($q.when(MOCK_PRIMARY_PAGE));
            mocks.displayConditionsFacade.getPrimaryPagesForVariationPageType.andReturn($q.when(MOCK_PRIMARY_PAGES));
        });
    ...
    describe('save', function() {
        beforeEach(function() {
            ...
            mocks.displayConditionsFacade.updatePage.andReturn($q.when(true));
        });

        it('should delegate to the facade to update page if dirty', function() {
            service.isDirty.andReturn(true);
        ...
        it('should not delegate to the facade if not dirty', function() {
            service.isDirty.andReturn(false);
            expect(service.save()).toBeResolvedWithData(true);

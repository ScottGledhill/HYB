 ...
 *
 */
.controller("navigationNodeEditorEntryListController", function($q, lodash, navigationEntryItemService, systemEventService, seValidationErrorParser, navigationNodeRestService, GENERIC_EDITOR_UNRELATED_VALIDATION_ERRORS_EVENT, GENERIC_EDITOR_LOADED_EVENT, NAV_ENTRY_EDITOR_ID) {
    ...
    this.dropdownItems = [{
        key: 'navigationmanagement.navnode.edit',
    ...
    var saveNodeErrorsEvent = function(key, _validationErrors) {
        var validationErrors = lodash.cloneDeep(_validationErrors);
        if (!_validationErrors.processed) {
            this.errorsMap = validationErrors.filter(function(validationError) {
            ...
            }).reduce(function(holder, next, array, index) {
            ...
            if (this.errorsMap && this.currentEntry && this.errorsMap[this.currentEntry.id]) {
                ...
                systemEventService.sendAsynchEvent(GENERIC_EDITOR_UNRELATED_VALIDATION_ERRORS_EVENT, this.errorsMap[this.currentEntry.id]);
            }
        }
        return $q.when();
    }.bind(this);
    ...
    // event listener 
    systemEventService.registerEventHandler(SAVE_ENTRY_EVENT, _pushEntryToList);
     ...
     */
    systemEventService.registerEventHandler(GENERIC_EDITOR_UNRELATED_VALIDATION_ERRORS_EVENT, saveNodeErrorsEvent);
    systemEventService.registerEventHandler(GENERIC_EDITOR_LOADED_EVENT, onEditorLoadEvent);

    // destroy listener on destroy controller scope
    this.$onDestroy = function() {
        systemEventService.unRegisterEventHandler(SAVE_ENTRY_EVENT, _pushEntryToList);
        systemEventService.unRegisterEventHandler(GENERIC_EDITOR_UNRELATED_VALIDATION_ERRORS_EVENT, saveNodeErrorsEvent);
        systemEventService.unRegisterEventHandler(GENERIC_EDITOR_LOADED_EVENT, onEditorLoadEvent);
    };
 ...
 */
.directive('navigationNodeEditorEntryList', function() {
    return {
        restrict: 'E',
        transclude: false,
        replace: false,
        templateUrl: 'web/features/cmssmarteditContainer/components/navigation/navigationNodeEditor/navigationNodeEditorComponents/navigationNodeEditorEntryListTemplate.html',

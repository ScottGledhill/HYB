...
angular.module('catalogInformationServiceModule', ['catalogServiceModule', 'productServiceModule', 'productCategoryServiceModule'])
    ...
    .service('seCatalogInformationService', function($q, lodash, catalogService, productService, productCategoryService, sharedDataService) {
        ...
        this.getProductCatalogsInformation = function() {
            if (this._parsedCatalogs) {
                return $q.when(this._parsedCatalogs);
            } else {
                return this._getSiteUID().then(function(siteUID) {
                    return catalogService.getProductCatalogsForSite(siteUID);
                }).then(function(catalogs) {
                    var result = {};
                    catalogs.map(function(catalog) {
                        if (!result[catalog.catalogId]) {
                        ...
                        var catalogVersion = {
                            ...
                            label: catalog.catalogVersion
                        };
                        result[catalog.catalogId].versions.push(catalogVersion);
                    });

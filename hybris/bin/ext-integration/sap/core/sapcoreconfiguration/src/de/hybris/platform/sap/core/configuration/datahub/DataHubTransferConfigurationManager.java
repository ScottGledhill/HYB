...
public class DataHubTransferConfigurationManager
{
	...
	public String getItemCode(final int typeCodeID)
	{
		if (typeCodeMap == null)
		{
			...
			for (final String typeCodeName : typeCodeKeySet)
			{
				...
				{
					LOG.warn("No item type code found for item code '" + typeCodeName + "'! Data won't be replicated to data hub after save.");
				}
				else
				{
	...
	public int getItemTypeCode(final String typeCodeName)
	{
		...
		{
			LOG.debug("Error while retrieving item code '" + typeCodeName + "'! Possibly caused by persistence manager timing issue.");
			return -1;
		}
	}
}

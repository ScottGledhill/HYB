...
public class ProductConfigRAOPopulatorTest
{
	...
	public void testPopulator_csticWithOutAssignableValues()
	{
		...
		assertEquals("Cstic Value Name is incorrect", ConfigurationRulesTestData.STRING_CSTIC_VALUE, csticRao.getAssignedValues()
				.get(0).getCsticValueName());
	}
	...
	public void testPopulator_numericCstic()
	{
		...
		final List<CsticRAO> csticRAOs = target.getCstics();
		final CsticRAO csticRao = csticRAOs.get(0);
		assertEquals("1500.0 in engine format is 1,500 in rules format (english locale!)", "1,500", csticRao.getAssignedValues()
				.get(0).getCsticValueName());
	}
	...
	public void testPopulator_assignableValueNumeric()
	{
		...
		final List<CsticRAO> csticRAOs = target.getCstics();
		final CsticRAO csticRao = csticRAOs.get(0);
		assertEquals("1500.0 final in engine format is 1,500 final in rules format (english locale!)", "1,500", csticRao
				.getAssignableValues().get(0).getCsticValueName());
	}
}

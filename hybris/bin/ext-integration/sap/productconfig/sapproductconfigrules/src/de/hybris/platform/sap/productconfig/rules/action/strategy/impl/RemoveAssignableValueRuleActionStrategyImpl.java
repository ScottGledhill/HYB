...
public class RemoveAssignableValueRuleActionStrategyImpl extends ProductConfigAbstarctRuleActionStrategy
{
	...
	protected boolean isActionPossible(final ConfigModel model, final AbstractRuleActionRAO action)
	{
		...
		if (actionApplicable)
		{
			actionApplicable = checker.checkValueUnassigned(model, action, getValueName(action), ACTION_DESCRIPTION);
		}
	...
	protected boolean executeAction(final ConfigModel model, final AbstractRuleActionRAO action)
	{
		...
		getCstic(model, action).removeAssignableValue(valueName);
		return false;
	}

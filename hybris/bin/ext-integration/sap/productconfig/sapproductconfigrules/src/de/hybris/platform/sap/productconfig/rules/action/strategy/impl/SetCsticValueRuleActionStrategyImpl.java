...
public class SetCsticValueRuleActionStrategyImpl extends ProductConfigAbstarctRuleActionStrategy
{
	...
	protected boolean isActionPossible(final ConfigModel model, final AbstractRuleActionRAO action)
	{
		...
		boolean actionApplicable = checker.checkCsticPartOfModel(model, action, ACTION_DESCRIPTION);
		if (actionApplicable)
		{
			final String value = getValueToSet(action, getCstic(model, action));
			actionApplicable &= checker.checkValueAssignable(model, action, value, ACTION_DESCRIPTION);
			actionApplicable &= checker.checkValueForamtable(model, action, value, getRulesFormator(), ACTION_DESCRIPTION);
		}

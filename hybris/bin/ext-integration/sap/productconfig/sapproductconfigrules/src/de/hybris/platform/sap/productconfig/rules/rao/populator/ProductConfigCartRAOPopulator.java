...
public class ProductConfigCartRAOPopulator implements Populator<CartModel, CartRAO>
{
	...
	{
		final String configIdForCartEntry = getSessionAccessService().getConfigIdForCartEntry(entry.getPk().toString());
		...
		if (null == configIdForCartEntry)
		{
			...
			{
				configModel = getProductConfigService().createConfigurationFromExternal(kbKey, externalConfiguration);
			}
		}
		else
		{

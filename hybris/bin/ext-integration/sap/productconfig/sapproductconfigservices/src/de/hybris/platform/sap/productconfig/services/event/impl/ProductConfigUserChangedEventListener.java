 ...
 * rule evaluation is re-triggered. So rule framework and configuration engine can immediately react on the user change.<br>
...
public class ProductConfigUserChangedEventListener extends AbstractEventListener<AfterSessionUserChangeEvent>
{
	...
	{
		logUserInfo(evt);
		getSessionAccessService().removeConfigurationModelEngineStates();
	}
	...
	protected void logUserInfo(final AfterSessionUserChangeEvent evt)
	{
		if (LOG.isDebugEnabled())
		{
			...
			LOG.debug("User Chnaged from " + evt.getPreviousUserUID() + " to " + newUser);
		}
	}

		...
		implements ConfigurationCartIntegrationFacade
{
	...
	{
		getConfigurationService().updateCartEntryProduct(cartItem, product, configData.getConfigId());
		fillCommerceCartParameterForUpdate(commerceCartParameter, cart, configData.getConfigId());
		getConfigurationService().updateCartEntryBasePrice(cartItem);
		...
		getCommerceCartService().calculateCart(commerceCartParameter);
		getConfigurationService().updateCartEntryExternalConfiguration(commerceCartParameter, cartItem);
		modelService.save(cartItem);
	}
	...
	public AbstractOrderEntryModel findItemInCartByPK(final PK cartItemPk)
	{
		...
		final Optional<AbstractOrderEntryModel> cartEntry = getCartService().getSessionCart().getEntries().parallelStream()

 ...
 */
public class CartProductVariantPopulator implements Populator<CartModel, CartData>
{
	...
	protected void populateCartEntry(final CartData target, final AbstractOrderEntryModel entry)
	{
		...
		if (product instanceof ERPVariantProductModel)
		{
			if (LOG.isDebugEnabled())
			{
				LOG.debug("CartItem with PK " + entry.getPk() + " is a product variant ==> populating DTO.");
			}
			...
			writeToTargetEntry(target, entry, features);
		}
		else
		{
			if (LOG.isDebugEnabled())
	 ...
	 *           List of features
	 */
	protected void writeToTargetEntry(final CartData target, final AbstractOrderEntryModel sourceEntry,
			final List<FeatureData> features)
	{
		final OrderEntryData targetEntry = target.getEntries() //
				.parallelStream() //
	...
	/**
	 * Generates a string of feature values separated with the value separator for configuration display in cart
	 *
	 * @param source
	 *           FeatureData
	 */
	protected String generateConfigurationValueLine(final FeatureData source)
	{
		...
		if (!CollectionUtils.isEmpty(featureValues))
		{
			for (int i = 0; i < featureValues.size(); i++)
			{
				if (i > 0)
				{
					builder.append(CartConfigurationDisplayPopulator.VALUE_SEPARATOR);
				}

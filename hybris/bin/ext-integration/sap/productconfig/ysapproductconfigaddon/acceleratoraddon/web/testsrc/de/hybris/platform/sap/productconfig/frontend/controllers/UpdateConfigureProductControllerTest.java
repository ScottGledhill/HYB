...
public class UpdateConfigureProductControllerTest extends AbstractProductConfigControllerTest
{
	...
	public void testConfigRemovedForwardToNewConfiguration() throws Exception
	{
		...
		assertTrue(view.getViewName().endsWith(UpdateConfigureProductController.CONFIG_ERROR_ROOT));
	...
	public void testHandleShowFullLongTextFlag_showLongText()
	{
		...
		assertTrue(uiStatusCstics.get(0).getId() + ": long text NOT shown, but expected", uiStatusCstics.get(0)
				.isShowFullLongText());
		assertFalse(uiStatusCstics.get(1).getId() + ": long text shown, but NOT expected", uiStatusCstics.get(1)
				.isShowFullLongText());
	}
	...
	public void testHandleShowFullLongTextFlag_hideLongText()
	{
		...
		assertNull("Focus Id should be resetted", configData.getFocusId());
		final List<UiCsticStatus> uiStatusCstics = updateData.getUiStatus().getGroups().get(0).getCstics();
		assertTrue(uiStatusCstics.get(1).getId() + ": long text NOT shown, but expected", uiStatusCstics.get(0)
				.isShowFullLongText());
		assertFalse(uiStatusCstics.get(0).getId() + ": long text shown, but NOT expected", uiStatusCstics.get(1)
				.isShowFullLongText());
	}
}

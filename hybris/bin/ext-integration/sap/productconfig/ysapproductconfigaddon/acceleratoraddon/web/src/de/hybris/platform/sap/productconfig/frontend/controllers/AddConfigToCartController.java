...
public class AddConfigToCartController extends AbstractProductConfigController
{
	...
	public String updateConfigInCart(@ModelAttribute(SapproductconfigaddonConstants.CONFIG_ATTRIBUTE)
	@Valid
	final ConfigurationData configData, final BindingResult bindingErrors, final Model model,
			final RedirectAttributes redirectModel) throws CommerceCartModificationException
	{
		try
		{
			return addConfigToCart(configData.getKbKey().getProductCode(), configData, bindingErrors, model, redirectModel);
		}
		catch (final CMSItemNotFoundException cinfe)
		{
			throw new CommerceCartModificationException("Root cause: CMSItemNotFoundException", cinfe);
		}
			...
			final RedirectAttributes redirectAttributes) throws CMSItemNotFoundException
	{
		...
		if (validationErrors)
		{
			final String redirectURL = REDIRECT_PREFIX + ROOT + productCode + SapproductconfigfrontendWebConstants.CONFIG_URL;
			return redirectURL;
		}
	...
	final String productCode, final String url) throws CMSItemNotFoundException
	{
		...
		final String newConfigId = getConfigCartFacade().copyConfiguration(oldConfigId, productCode);

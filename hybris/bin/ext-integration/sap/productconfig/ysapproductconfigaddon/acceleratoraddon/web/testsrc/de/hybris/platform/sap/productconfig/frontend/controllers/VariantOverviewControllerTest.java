...
@UnitTest
public class VariantOverviewControllerTest extends AbstractProductConfigControllerTest
{
	...
	{
		final ConfigurationOverviewData overviewData = new ConfigurationOverviewData();
		given(productData.getCode()).willReturn(PRODUCT_CODE);

		final OverviewUiData uiData = classUnderTest.prepareOverviewUiData(overviewData, productData);
		assertEquals(overviewData, uiData.getConfigurationOverviewData());
		assertEquals(OverviewMode.VARIANT_OVERVIEW, uiData.getOverviewMode());
	}
	...
	protected void initializeFirstCall() throws Exception
	{
		...
		given(configurationOverviewFacade.getOverviewForProductVariant(Mockito.eq(PRODUCT_CODE), Mockito.any())).willReturn(
				overviewData);

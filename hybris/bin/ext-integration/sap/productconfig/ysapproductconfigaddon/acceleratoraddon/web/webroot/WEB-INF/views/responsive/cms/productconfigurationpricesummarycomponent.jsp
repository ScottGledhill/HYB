...
<div id="priceSummary" class="cpq-price-sum col-xs-12 col-sm-6 col-sm-offset-6">
	<c:if test="${showPriceDetails and !config.priceSummaryCollapsed and !(variantOverview)}">
	...
	<div class="cpq-price-sum-total">
		...
		<c:choose>
			<c:when test="${variantOverview}">
				<div id="currentTotalValue" class="cpq-price-value cpq-price-total">${product.price.formattedValue}</div>
			</c:when>
			<c:otherwise>
...
<div id="addToCartCol" class="cpq-addToCart col-xs-12 col-sm-6 <c:if test="${empty overviewUiData}">col-sm-offset-6</c:if>">
	<c:choose>
		<c:when test="${not empty overviewUiData and overviewUiData.overviewMode eq 'CONFIGURATION_OVERVIEW'}">

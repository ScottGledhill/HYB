...
public class C4CTicketFacadeImpl implements TicketFacade
{
	...
	public TicketData createTicket(final TicketData ticket)
	{
		...
		try
		{
			...
			final HttpEntity<String> entity = new HttpEntity<>(
					getJacksonObjectMapper().writeValueAsString(getDefaultC4CTicketConverter().convert(ticket)), headers);
	...
	public TicketData updateTicket(final TicketData ticket)
	{
		...
		try
		{
			...
			final HttpHeaders statusUpdateHeaders = getHttpHeaderUtil()
					.addBatchHeaders("PATCH ServiceTicketCollection('" + ticket.getId() + "') HTTP/1.1");

			final HttpHeaders messageUpdateHeaders = getHttpHeaderUtil()
					.addBatchHeaders("POST ServiceTicketCollection('" + ticket.getId() + "')/Notes HTTP/1.1");
			...
			final HttpEntity<String> statusEntity = new HttpEntity<>(
					getJacksonObjectMapper().writeValueAsString(getDefaultC4CTicketConverter().convert(ticket)), statusUpdateHeaders);

			final HttpEntity<String> messageEntity = new HttpEntity<>(
					getJacksonObjectMapper().writeValueAsString(getUpdateMessageConverter().convert(ticket)), messageUpdateHeaders);
			...
			final ResponseEntity<MultiValueMap> result = getRestTemplate().exchange(uri, HttpMethod.POST, requestEntity,
					MultiValueMap.class);
	...
	public TicketData getTicket(final String ticketId)
	{
		...
		final UriComponentsBuilder builder = UriComponentsBuilder
				...
				.queryParam(Customerticketingc4cintegrationConstants.FILETR_SUFFIX,
						(getSitePropsHolder().isB2C()
								? String.format("ExternalCustomerID eq '%s'", getCustomerFacade().getCurrentCustomer().getCustomerId())
								: String.format("ExternalContactID eq '%s'", getCustomerFacade().getCurrentCustomer().getCustomerId()))
	...
	public SearchPageData<TicketData> getTickets(final PageableData pageableData)
	{
		...
		try
		{
			...
			final ODataListResultsData oDataListResultsData = getJacksonObjectMapper()

...
public class C4CTicketFacadeImplTest extends ServicelayerTest
{
	...
	public void shouldGetTicket() throws IOException
	{
		...
		Mockito.when(restTemplate.exchange(Mockito.any(URI.class), Mockito.eq(HttpMethod.GET), Mockito.eq(entity),
	...
	public void shouldReturnEmptySearchResultWhenGetTicketsThrowIOException() throws IOException
	{
		...
		Mockito.when(c4cBaseFacade.convertPageData(Collections.emptyList(), ticketConverter, pageableData, 0))
	...
	public void shouldEnrichHeaders()
	{
		...
		Mockito.when(returnedHttpHeader.get(Customerticketingc4cintegrationConstants.RESPONSE_COOKIE_NAME))
				.thenReturn(Arrays.asList("response-cookie-name"));
		Mockito.when(Boolean.valueOf(returnedHttpHeader.containsKey(Customerticketingc4cintegrationConstants.TOKEN_NAMING)))
				.thenReturn(Boolean.TRUE);
		Mockito.when(returnedHttpHeader.get(Customerticketingc4cintegrationConstants.TOKEN_NAMING))
				.thenReturn(Arrays.asList("token-naming"));

		final HttpHeaders headers = new HttpHeaders();
		final HttpHeaders enrichedHeaders = httpHeaderUtilMock.enrichHeaders(headers, SITE_ID);

		Mockito.verify(restTemplate)
				.exchange(Customerticketingc4cintegrationConstants.URL + Customerticketingc4cintegrationConstants.TICKETING_SUFFIX
	...
	public void shouldUpdateTicketWithOpenStatus() throws JsonProcessingException
	{
		...
		Mockito.when(restTemplate.exchange(Mockito.any(URI.class), Mockito.eq(HttpMethod.POST), Mockito.eq(requestEntity),

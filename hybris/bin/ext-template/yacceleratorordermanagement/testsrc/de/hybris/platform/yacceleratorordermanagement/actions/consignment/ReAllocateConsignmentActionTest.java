...
public class ReAllocateConsignmentActionTest
{
	...
	private DeclineEntry autoEntry;
	private DeclineEntry manualEntry;
	...
	public void setup()
	{
		...
		externalWarehouseDeclineActionsMap = spy(new HashMap<>());
		...
		final Collection<BusinessProcessParameterModel> contextParameters= spy(new ArrayList<>());
		...
		when(orderBusinessProcessService.createProcess(anyString(),eq(YAcceleratorOrderManagementConstants.CONSIGNMENT_SUBPROCESS_NAME))).thenReturn(newConsignmentProcessModel);

...
public class ReturnIntegrationTest extends BaseAcceleratorSourcingIntegrationTest
{
	...
	@Test
	public void shouldReturnSuccessButNotRestock_SingleEntry_ReturnOnline_SendReturnLabelEmail() throws RetryLaterException, InterruptedException,
			ImpExException {
		//given create return
		emailAddresses.polo();
		setWarehouseIsAllowRestock(warehouses.Boston());
		...
		importCsv(
				"/yacceleratorordermanagement/test/impex/email-content.impex",
				WarehousingTestConstants.ENCODING);
		importCsv(
				"/yacceleratorordermanagement/test/impex/email-content_en.impex",
				WarehousingTestConstants.ENCODING);
		...
		ReturnProcessModel emailProcess = returnRequest.getReturnProcess().stream().filter(p -> p.getCode().contains("sendReturnLabelEmail"))
				.iterator().next();
	...
	@Test(expected = IllegalArgumentException.class)
	public void shouldReturnSuccessful_RestockFromExternalWarehouse_SecondAttemptExpectedQuantityTooHigh_Shipping() throws InterruptedException
	{
		//when
	...
	@Test
	public void shouldReturnSuccessful_RestockFromExternalWarehouse_CancelAndSecondAttemptExpectedQuantity_Shipping() throws InterruptedException
	{
		//when
		createOrderWithOneEntry_ShippingOrder_ExternalWarehouse();
		ReturnRequestModel returnRequest = returnUtil.createDefaultReturnRequest(order, 2L, ReturnAction.HOLD,

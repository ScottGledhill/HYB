...
import de.hybris.platform.warehousing.jalo.SourcingBan;
...
public class DeclineIntegrationTest extends BaseAcceleratorSourcingIntegrationTest
{
	...
	public void shouldManualReallocate_DamagedEntry_BanExternalWarehouse() throws InterruptedException
	{
		...
		assertEquals(Boolean.TRUE,
				verifyOrderAndConsignment.verifyConsignment_Camera(order, CODE_MONTREAL_EXTERNAL, CAMERA_QTY, CAMERA_QTY, Long.valueOf(0L)));
		assertEquals(Boolean.TRUE,
				verifyOrderAndConsignment.verifyConsignment_Camera(order, CODE_BOSTON, Long.valueOf(0L), CAMERA_QTY, CAMERA_QTY));

		//verify the location get banned
		final Collection<SourcingBanModel> sourcingBans = sourcingBanService.getSourcingBan(Collections.singletonList(externalMontrealWarehouse));
	...
	public void shouldExcludeBanWarehouseDuringATPCalculation() throws InterruptedException
	{
		...
		//Then verify the location get banned
		final Collection<SourcingBanModel> sourcingBans = sourcingBanService.getSourcingBan(Collections.singletonList(warehouses.Montreal()));

...
public class DefaultCxServiceCmsIntegrationTest extends ServicelayerTest
{
	...
	public void testPersonalizationCalculationSequence()
	{
		...
		user.setCxResults(new ArrayList<>());
		...
		//step 1 : calculation should store on user, but not in session
	...
	public void testPersonalizationCalculationSequenceSessionOnly()
	{
		...
		Assert.assertTrue(user.getCxResults().size() == 0);
	}
	...
	public void testPreviewPersonalizationCalculationSequence()
	{
		...
		//step 1 : preview only loads results in session but not on userMOdel
	...
	protected void assertResultOnUser(final UserModel user)
	{
	...
	public void testUpdateCustomerExperienceAsync() throws InterruptedException
	{
		//user cleanup
		final UserModel user = userService.getUserForUID(USER_ID);
		modelService.removeAll(user.getCxResults());
		user.setCxResults(new ArrayList<>());
		modelService.save(user);
		...
		//when
		final CxPersonalizationProcessModel process = cxService.startPersonalizationCalculationProcess(user,
		...
		Awaitility.await().atMost(60, TimeUnit.SECONDS).until(new Callable<Boolean>()
		{
			...
			public Boolean call() throws Exception
			{
				...
				final BusinessProcessService businessPorcessService = Registry.getApplicationContext()
						.getBean(BusinessProcessService.class);
				final ModelService modelService = Registry.getApplicationContext().getBean(ModelService.class);
				final BusinessProcessModel localProcess = businessPorcessService.getProcess(processCode);
		...
		Assert.assertTrue(user.getCxResults().size() == 1);
		Assert.assertNotNull(user.getCxResults().iterator().next().getResults());
	}
